...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
	//make something when button press
	//time limits are modified because of my too slow fingers
	if(timer_counter < 500){
		timer_counter = 0;
		//short_signal++;
		character = character << 1;
		printf("short signal %d\n" , character);
	} else if( 500 < timer_counter && timer_counter < 1000 ){
		timer_counter = 0;
		//long_signal++;
		character = (character << 1)|1;
		printf("long signal %d\n" , character);
	} else if(timer_counter >= 1000){
		printf("end of the character\n");
		timer_counter = 0;
	}
}
